...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest)
{
    ...
    tx1.vout[0].nValue = 10 * COIN;
    AddToMempool(pool, entry.Fee(100000LL).FromTx(tx1));
    ...
    AddToMempool(pool, entry.Fee(50000LL).FromTx(tx2));
    ...
    AddToMempool(pool, entry.Fee(200000LL).FromTx(tx3));
    ...
    AddToMempool(pool, entry.Fee(70000LL).FromTx(tx4));
    AddToMempool(pool, entry.Fee(10000LL).FromTx(tx5));
    AddToMempool(pool, entry.Fee(11000LL).FromTx(tx6));
    AddToMempool(pool, entry.Fee(90000LL).FromTx(tx7));
