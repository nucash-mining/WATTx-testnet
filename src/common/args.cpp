...
const char * const BITCOIN_CONF_FILENAME = "wattx.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\WATTx
    //   new: C:\Users\Username\AppData\Local\WATTx
    // macOS: ~/Library/Application Support/WATTx
    // Unix-like: ~/.wattx
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "WATTx";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "WATTx";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/WATTx";
#else
    // Unix-like
    return pathRet / ".wattx";
#endif
#endif
}
